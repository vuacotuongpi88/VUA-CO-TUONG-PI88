...
function isTerminalWithdrawStatus(status) {
  const s = String(status || "").trim();
  return (
    s === "done" ||
    s === "rejected_admin" ||
    s === "failed" ||
    s === "manual_repair_failed"
  );
}

function isPendingAdminStatus(status) {
  return String(status || "").trim() === "pending_admin";
}

function isActiveProcessingStatus(status) {
  const s = String(status || "").trim();
  return (
    s === "initiated" ||
    s === "auto_processing" ||
    s === "chain_submitted" ||
    s === "internal_deduct_failed_after_chain_success"
  );
}
...
async function inspectWithdrawQueue(db, safeWalletKey) {
  ...
  snap.forEach((child) => {
    ...
    if (!pendingAdminRequest && isPendingAdminStatus(status)) {
      pendingAdminRequest = { key: child.key, value: v };
    }

    const otherWallet = String(v.oldPendingWalletKey || "").trim();
    if (otherWallet && safeKey(otherWallet) !== safeWalletKey) {
      lastOtherWalletHint = otherWallet;
    }
  });
