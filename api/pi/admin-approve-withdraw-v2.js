...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    stage = "request-read";
    requestRef = db.ref(`piWithdrawRequestsV2/${requestId}`);
    ...
    if (!requestVal || typeof requestVal !== "object") {
      return res.status(404).json({
        ok: false,
        error: "Không tìm thấy withdraw request V2."
      });
    }
    ...
    const memo = String(
      requestVal.memo || `Rut ${amount.toFixed(2)} Pi v2`
    ).trim();
    ...
    await requestRef.update(
      cleanForFirebase({
        ...
        approvedNote: adminNote || "Admin duyệt lệnh rút V2",
        approvedAt: nowMs(),
        updatedAt: nowMs()
      })
    );
