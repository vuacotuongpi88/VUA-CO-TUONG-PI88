...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (walletVal.piVerified !== true || !String(walletVal.piUid || "").trim()) {
      return res.status(400).json({
        ok: false,
        error: "Tài khoản này chưa liên kết Pi Browser."
      });
    }
    ...
    stage = "lock";
    lockRef = db.ref(`wallets/${safeWalletKey}/withdrawLockV2`);
    ...
    stage = "queue-check";
    const queueInfo = await inspectWithdrawQueueV2(db, safeWalletKey);
    ...
    if (queueInfo.pendingAdminRequest) {
      ...
      return res.status(200).json({
        ...
        message: "Ví này đang có lệnh chờ duyệt. Không cần bấm thêm."
      });
    }

    const riskFlags = buildRiskFlags({ amount, queueInfo });
    const queueForAdmin = shouldQueueForAdmin(riskFlags);

    stage = "create-request";
    requestRef = db.ref("piWithdrawRequestsV2").push();
    withdrawId = requestRef.key || "";

    const memo = `Rut ${Number(amount).toFixed(2)} Pi v2`;

    await requestRef.set(
      cleanForFirebase({
        status: queueForAdmin ? "pending_admin" : "initiated",
        type: "wallet_withdraw_v2",
    ...
    if (queueForAdmin) {
      ...
      const reasons = [];
      if (riskFlags.overAutoMax) reasons.push(`Vượt ngưỡng auto ${CONFIG.AUTO_WITHDRAW_MAX} Pi`);
      if (riskFlags.burstRequests) reasons.push("Bấm rút quá nhanh");
      if (riskFlags.hasPendingAdmin) reasons.push("Đã có lệnh chờ duyệt");
      if (riskFlags.hasActiveRequest) reasons.push("Đang có lệnh xử lý");
      ...
      return res.status(200).json({
        ...
        withdrawId,
        message:
          "Lệnh rút đã được đưa vào hàng duyệt. " +
    ...
    if (!deductOk) {
      ...
      return res.status(200).json({
        ...
        withdrawId,
        txid,
        message: "Pi đã về ví ngoài nhưng app chưa trừ nội bộ. Đã đánh dấu để xử lý tiếp."
      });
    }
